...
namespace PPTVideo
{
    public partial class FormAudio : Form
    {
        ...
        public string GetEnhancers(bool enhancers)
        {
            if (enhancers)
            {
                return "gfpgan";
        ...
        public string GetEnhancersMotion(bool enhancers)
        {
            if (enhancers)
            {
                return "Enhanced";
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (global.digitalMotion == 1)
            {
                ...
                if (m_preModel)
                {
                    ...
                    {
                        global.m_index = m_index;

                        //SadTalker参数
                        var SadTalkerParmes = new
                        {
                            enhancer = GetEnhancers(global.m_enhancer),
                            expression_scale = global.m_expression_scale
                        };
                        ...
                        {
                            User = Program.Flask.m_user,
                            Index = global.m_index   //0是男,1是女 string类型
                        };
            ...
            else if (global.digitalMotion == 2)
            {
                ...
                if (m_preModel)
                {
                    ...
                    else
                    {
                        ...
                        //Wav2Lip参数
                        var wav2LipParmes = new
                        {
                            quality = GetEnhancersMotion(global.m_enhancer)
                        ...
                        {
                            User = Program.Flask.m_user,
                            Wav2Lip_Config = wav2LipParmes,
                        };
                        string vitsJson = JsonConvert.SerializeObject(VitsParmes);
                        string wav2lipJson = JsonConvert.SerializeObject(Config);

                        //配置VITS模型
                        if (Program.Flask.SendSelectVITSModel(vitsJson))   
                        {
                            //配置wav2lip参数
                            if (Program.Flask.SendWav2LipConfig(wav2lipJson))
                            {
                                global.vitsConfig = false;
                                MessageBox.Show("提交成功", "提示");
                            }
                ...
                else if (m_trainModel)
                {
                    if (textBox11.Text != "")
                    {
                        if (m_refWav != "")
                        {
                            ...
                            //配置VITS模型
                            if (Program.Flask.SelectTrainVitsModel(vitsJson))   
                            {
                                //配置参数
                                if (Program.Flask.SendWav2LipConfig(wav2lipJson))
                                {
        ...
        {
            if (global.modelName != "没有模型可选")
            {
                try
                {
                    m_modelNane = comboBox1.SelectedItem.ToString();
                    if (textBox11.Text != "")
                    {
                ...
                {
                    MessageBox.Show("请选择模型", "提示");
                }
            }
            else
            {
                MessageBox.Show("没有可用的模型" , "提示");
            }
        ...
        private void cbTrainAi_CheckedChanged(object sender, EventArgs e)
        {
            if (cbTrainAi.Checked)
            {
                ...
                m_preModel = false;
                m_trainModel = true;
                comboBox1.Items.Add(global.modelName);
            }
        }
